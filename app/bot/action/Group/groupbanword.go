...
func App_group_ban_word_list(self_id, group_id, user_id int64, message string, Type int, groupmember map[string]interface{}, groupfunction map[string]interface{}) {
	...
	if len(kv) > 0 {
		...
		api.Sendgroupmsg(self_id, group_id, message+"列表为："+strings.Join(word, ","), true)
	} else {
		api.Sendgroupmsg(self_id, group_id, message+"列表为空，可以使用“acfur屏蔽”来查看设定方法", true)
	}
}
...
func App_group_ban_word_set(self_id, group_id, user_id int64, message string, message_id int64, groupmember map[string]interface{}, groupfunction map[string]interface{}) {
	if len(message) > 2 {
		...
		if len(new_str) < 1 {
			api.Sendgroupmsg(self_id, group_id, "屏蔽词设定需要大于1位", true)
			return
		}
		switch Type {
		case "1":
			...
			if len(data) > 0 {
				if GroupBanWordModel.Api_update(group_id, new_str, data["is_kick"], true, true) {
					api.Sendgroupmsg(self_id, group_id, "修改成功", true)
				} else {
		...
		case "-":
			...
			if len(data) > 0 {
				if GroupBanWordModel.Api_delete(group_id, new_str) {
					api.Sendgroupmsg(self_id, group_id, "屏蔽词已经删除", true)
				} else {
					api.Sendgroupmsg(self_id, group_id, "屏蔽词删除失败", true)
				}
			} else {
				api.Sendgroupmsg(self_id, group_id, "没有找到对应的屏蔽词，也许已经删除了呢？", true)
			}
			break
		...
		case "+":
			...
			if len(strs) < 2 {
				api.Sendgroupmsg(self_id, group_id, "设定错误，你可以使用：acfur屏蔽+屏蔽词#处罚，例如acfur屏蔽+触发词#T出撤回", true)
			} else {
				...
				if len(data) > 0 {
					if GroupBanWordModel.Api_update(group_id, strs[0], strings.Contains(strs[1], "T出"), strings.Contains(strs[1], "屏蔽"), strings.Contains(strs[1], "撤回")) {
						api.Sendgroupmsg(self_id, group_id, "屏蔽词更新成功", true)
					} else {
					...
					if GroupBanWordModel.Api_insert(group_id, user_id, strs[0], 0, strings.Contains(strs[1], "T出"),
						...
						api.Sendgroupmsg(self_id, group_id, "屏蔽词"+strs[0]+"添加失败", true)
					}
				}
		...
		default:
			api.Sendgroupmsg(self_id, group_id, "如需添加，请使用acfur屏蔽(+)1/2/3/+/-/=(+)屏蔽词即可，如需其他帮助，请发送acfur屏蔽来查看添加方法", true)
			break
		}
	} else {
		api.Sendgroupmsg(self_id, group_id, "屏蔽词命令有误，请发送acfur屏蔽查看如何使用屏蔽词", true)
	}
}
