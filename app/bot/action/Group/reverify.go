...
func reverify(self_id, group_id, user_id any, send_to_message string, kick, force bool) (string, error) {
	...
	go apipost.Api{}.SetGroupBan(self_id, group_id, member["user_id"], 0)
	if len(user) > 0 || force {
		if len(user) < 1 {
		...
		go apipost.Api{}.Sendgroupmsg(self_id, group_id, at+"你已被临时解禁，请在120秒内在群内输入验证码数字：\n"+Calc.Any2String(num), true)
		go func(self_id, group_id, user_id any, kick bool) {
			...
			if err != nil {
				...
				if ok {
					go apipost.Api{}.Sendgroupmsg(self_id, group_id, at+"看起来你没有完成活人验证，我现在将你加入永久小黑屋，但是你依然可以让其他管理员帮你解除", true)
					if kick {
