	...
	if left_time > 0 {
		...
		if groupbal != nil {
			...
			if balance_left >= 0 {
				...
				go apipost.Api{}.Sendgroupmsg(self_id, group_id, at+Calc.Any2String(time+1)+"这是你第:"+"次扣分，扣除"+Calc.Any2String(balance_decr)+"分\n"+"本次扣分原因："+reason+"\n你还剩下："+
					""+Calc.Any2String(balance_left)+"分", auto_retract)
				return
			}
		}
		go apipost.Api{}.Sendgroupmsg(self_id, group_id, at+"这是你第:"+Calc.Any2String(time+1)+"次，接受惩罚\n"+"本次惩罚原因："+reason+"\n你还剩下："+Calc.Any2String(left_time)+"点生命值", auto_retract)
		apipost.Api{}.SetGroupBan(self_id, group_id, user_id, float64(groupfunction["ban_time"].(int64))*math.Pow10(int(time)))
	} else {
...
func App_kick_user(self_id, group_id, user_id any, auto_retract bool, groupfunction map[string]any, reason string) {
	...
	if auto_kick_out == 1 {
		...
		apipost.Api{}.SetGroupKick(self_id, group_id, user_id, false)
		if len(gm) > 0 {
			nickname := Calc.Any2String(gm["nickname"])
			go apipost.Api{}.Sendgroupmsg(self_id, group_id, nickname+"被T出"+str+"，原因为："+reason, auto_retract)
		} else {
			go apipost.Api{}.Sendgroupmsg(self_id, group_id, Calc.Any2String(user_id)+"被T出"+str+"，原因为："+reason, auto_retract)
		}
	} else {
		if len(GroupBanPermenentModel.Api_find(group_id, user_id)) > 0 {
			...
			if GroupBanPermenentModel.Api_insert(group_id, user_id, time.Now().Unix()+app_conf.Auto_ban_time-86400) {
				at := service.Serv_at(user_id)
				apipost.Api{}.SetGroupBan(self_id, group_id, user_id, app_conf.Auto_ban_time)
				go apipost.Api{}.Sendgroupmsg(self_id, group_id, at+"你已经低于生命值，现在将你加入永久禁言列表，仅允许管理员解禁", auto_retract)
			}
		}
	}
}
	...
	if len(group_list_data) > 0 {
		...
		if group_list_data["max_member_count"].(int64) < group_member_count {
			...
			if len(group_member_datas) > 0 {
				go apipost.Api{}.Sendgroupmsg(self_id, group_id, "本群将被清除"+Calc.Any2String(len(group_member_datas))+
				...
				for _, data := range group_member_datas {
					ok, err := apipost.Api{}.SetGroupKick(self_id, group_id, data["user_id"], false)
					if err != nil {
						fmt.Println(err)
					} else {
						if ok {
				...
				go apipost.Api{}.Sendgroupmsg(self_id, group_id, "没有需要清理的人", true)
			}
		} else {
			go apipost.Api{}.Sendgroupmsg(self_id, group_id, "未达到清理下限无需调用", true)
		}
	} else {
		go apipost.Api{}.Sendgroupmsg(self_id, group_id, "未找到当前群信息", true)
	}
}
