...
func App_group_function_set(self_id, group_id, user_id int64, message string, groupfunction map[string]any) {
	...
	if i1 == i2 {
		AutoMessage(self_id, group_id, user_id, MessageBuilder.IMessageBuilder{}.New().Text("如果需要设定，请使用acfur设定设定内容：设定结果，例如\r\n\"acfur设定入群欢迎：开"), groupfunction)
		return
	}
	strs := []string{}
	if (i1 < i2 || i2 == -1) && i1 != -1 {
	...
	if len(detail) > 0 {
		...
		case "int":
			if len(set) > 0 {
				...
				if err != nil {
					AutoMessage(self_id, group_id, user_id, MessageBuilder.IMessageBuilder{}.New().Text(name+"只能设定为数字整数,请调整为数字整数"), groupfunction)
					return
				} else {
					value = i
				}
			} else {
				AutoMessage(self_id, group_id, user_id, MessageBuilder.IMessageBuilder{}.New().Text(name+"的设定有误，例子：acfur设定"+name+":"+"数字"), groupfunction)
				return
			}
			break
		...
		case "string":
			if len(set) > 0 {
				...
				AutoMessage(self_id, group_id, user_id, MessageBuilder.IMessageBuilder{}.New().Text(name+"的设定有误，例子：acfur设定"+name+":"+"你要设定的文字"), groupfunction)
			}
			break

		default:
			AutoMessage(self_id, group_id, user_id, MessageBuilder.IMessageBuilder{}.New().Text(name+"需要有设定项，你可以使用acfur设定"+name+":"+"设定值，对该功能进行设定"), groupfunction)
			return
		}
		if GroupFunctionModel.Api_update(group_id, detail["key"], value) {
			AutoMessage(self_id, group_id, user_id, MessageBuilder.IMessageBuilder{}.New().Text(name+"设定成功为"+":"+set), groupfunction)
		} else {
			AutoMessage(self_id, group_id, user_id, MessageBuilder.IMessageBuilder{}.New().Text(name+"设定失败"), groupfunction)
		}
	} else {
		AutoMessage(self_id, group_id, user_id, MessageBuilder.IMessageBuilder{}.New().Text(name+"没有找到对应的设定项目，\r\n如果需要设定，请使用acfur设定设定内容：设定结果，例如\r\nacfur设定入群欢迎：开"), groupfunction)
	}
}
