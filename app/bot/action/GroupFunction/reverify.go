...
func reverify(self_id, group_id, user_id int64, send_to_message string, kick, force bool) (string, error) {
	...
	if len(user) > 0 || force {
		...
		msg := MessageBuilder.IMessageBuilder{}.New().At(member["user_id"]).Text("你已被临时解禁，请在120秒内在群内输入验证码数字：\n" + Calc.Any2String(num))
		...
		go func(self_id, group_id, user_id int64, kick bool) {
			...
			if err != nil {
				...
				if ok {
					msg := MessageBuilder.IMessageBuilder{}.New().At(member["user_id"]).Text("看起来你没有完成活人验证，我现在将你加入永久小黑屋，但是你依然可以让其他管理员帮你解除")
					go iapi.Api.SendGroupMsg(self_id, group_id, msg, true)
					if kick {
