...
func App_userLogin(self_id int64, user_id, group_id int64, message string) {
	...
	uname := ""
	nickname, err := iapi.Post{}.GetStrangerInfo(self_id, user_id, false)
	if err != nil {
	...
	if len(usermember) > 0 {
		if UserMemberModel.Api_update_all(user_id, uname, rand) {
			iapi.Post{}.Sendprivatemsg(self_id, user_id, group_id, "您的登录密码：\r\n"+Calc.Int2String(rand), false)
		} else {
			iapi.Post{}.Sendprivatemsg(self_id, user_id, group_id, app_default.Default_error_alert, false)
		}
	} else {
