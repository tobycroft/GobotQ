...
func group_check(self_id, user_id, group_id int64) {
	...
	if len(groupinfo) < 1 {
		...
		Redis.String_set("__groupinfo__"+Calc.Int642String(group_id)+"_"+Calc.Int642String(user_id), groupinfo, 60)
	}
	userinfo := GroupMemberModel.Api_find(group_id, user_id)
	if len(userinfo) < 1 {
		Group.App_refreshmember(self_id, group_id)
	} else {
		Redis.String_set("__userinfo__"+Calc.Int642String(group_id)+"_"+Calc.Int642String(user_id), groupinfo, 60)
	}
}
