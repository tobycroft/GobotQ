...
func groupHandle_acfur(self_id, group_id, user_id int64, message_id int64, new_text, message, raw_message string, sender _Sender, groupmember map[string]any, groupfunction map[string]any) {
	...
	case "":
		go apipost2.Api{}.Sendgroupmsg(self_id, group_id, app_default.Default_welcome, true)
		break
	...
	case "测试撤回":
		...
		go func(ret apipost2.Struct_Retract) {
			apipost2.Retract_instant <- ret
		}(ret)
		break
...
func groupHandle_acfur_other(Type string, self_id, group_id, user_id, message_id int64, message, raw_message string, sender _Sender, groupmember map[string]any, groupfunction map[string]any) {
	...
	case "自动回复":
		go apipost2.Api{}.Sendgroupmsg(self_id, group_id, message, auto_retract)
		break
		...
		go func(selfId, groupId, userId any, groupFunction gorose.Data) {
			...
			if err != nil {
				...
				if code == message {
					...
					go apipost2.Api{}.Sendgroupmsg(selfId, groupId, service.Serv_at(userId)+"验证成功"+str, true)
				} else {
					go apipost2.Api{}.Sendgroupmsg(selfId, groupId, service.Serv_at(userId)+"你的输入不正确，需要输入："+Calc.Any2String(code), true)
				}
			}
