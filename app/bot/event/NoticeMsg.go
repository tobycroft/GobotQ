...
func (em Notice) NoticeMsg() {
		...
		case "set":
			if user_id == self_id {
				if GroupMemberModel.Api_update_type(group_id, user_id, "admin") {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "Acfur-On，已获取权限，可使用acfurhelp查看功能", auto_retract)
				} else {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "Acfur-On，已获取权限，数据故障，请使用acfur刷新人数来更新信息", auto_retract)
				}
			} else {
				if GroupMemberModel.Api_update_type(group_id, user_id, "admin") {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "恭喜上位"+service.Serv_at(user_id), auto_retract)
					...
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "恭喜上位,但是权限变动失败", auto_retract)
				}
			}
		...
		case "unset":
			if user_id == self_id {
				if GroupMemberModel.Api_update_type(group_id, user_id, "member") {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "Acfur-Off，权限已回收，将在2小时内退群", auto_retract)
				} else {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "Acfur-Off，权限已回收，数据故障", auto_retract)
				}
			} else {
				if GroupMemberModel.Api_update_type(group_id, user_id, "member") {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "管理员列表更新", auto_retract)
				} else {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "管理员权限变动失败", auto_retract)
				}
			}
			break
	...
	case "group_increase":
		if user_id == self_id {
			...
			if groupfunction["auto_hold"].(int64) == 1 {
				...
				go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, at+"请在120秒内在群内输入验证码数字：\n"+Calc.Any2String(num), true)
				go func(selfId, groupId, userId any) {
					...
					if err != nil {
						...
						if ok {
							go apipost.ApiPost{}.Sendgroupmsg(selfId, groupId, at+"看起来你没有完成活人验证，我现在将你加入永久小黑屋，但是你依然可以让其他管理员帮你解除", true)
							apipost.ApiPost{}.SetGroupBan(selfId, groupId, userId, app_conf.Auto_ban_time)
						}
					}
				...
				if groupfunction["auto_welcome"].(int64) == 1 {
					if groupfunction["welcome_at"].(int64) == 1 {
						go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, service.Serv_at(user_id)+Calc.Any2String(groupfunction["welcome_word"]), auto_retract)
					} else {
						go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, Calc.Any2String(groupfunction["welcome_word"]), auto_retract)
					}
				} else {
					if groupfunction["join_alert"].(int64) == 1 {
						go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "成员+1", auto_retract)
					}
				}
			}
			...
			if groupfunction["auto_card"].(int64) == 1 {
				...
				if groupfunction["auto_card_insert"] == 1 && err == nil {
					go apipost.ApiPost{}.Setgroupcard(self_id, group_id, user_id, comment)
				} else {
					go apipost.ApiPost{}.Setgroupcard(self_id, group_id, user_id, groupfunction["auto_card_value"])
				}
			}

			go func(selfId, groupId, userId int64, autoRetract bool) {
				//将这个新加群的用户单条加入数据库
				member, err := apipost.ApiPost{}.GetGroupMemberInfo(selfId, groupId, userId)
				if err != nil {
					...
					if !GroupMemberModel.Api_insert(mb) {
						go apipost.ApiPost{}.Sendgroupmsg(selfId, groupId, "群成员数据增加失败", autoRetract)
					}
				}
		...
		case "leave":
			if groupfunction["exit_to_black"].(int64) == 1 {
				...
				if groupfunction["exit_alert"].(int64) == 1 {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, Calc.Any2String(user_id)+"退群，已加入本群黑名单", auto_retract)
				}
			} else {
				if groupfunction["exit_alert"].(int64) == 1 {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "成员-1", auto_retract)
		...
		case "kick":
			...
			if groupfunction["kick_to_black"].(int64) == 1 {
				...
				if GroupKickModel.Api_insert(self_id, group_id, user_id, jsonmsg) {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "群成员"+Calc.Any2String(user_id)+"T出报告已经生成，并已加入黑名单，请在APP中查看", auto_retract)
				} else {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "群成员"+Calc.Any2String(user_id)+"T出报告生成失败，但已加入黑名单", auto_retract)
				}
			} else {
				if GroupKickModel.Api_insert(self_id, group_id, user_id, jsonmsg) {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "群成员T出报告已经生成，请在APP中查看", auto_retract)
				} else {
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, "群成员T出报告生成失败", auto_retract)
				}
			}
			break
	...
	case "group_ban":
		...
		case "ban":
			if em.Duration >= 2505600 {
				if len(GroupBanPermenentModel.Api_find(group_id, user_id)) > 0 {
					...
					GroupBanPermenentModel.Api_insert(group_id, user_id, time.Now().Unix()+app_conf.Auto_ban_time-86400)
					go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, service.Serv_at(user_id)+"你进入永久小黑屋，可联系群管解除", auto_retract)
				}
			}
			break
		...
		case "lift_ban":
			if len(GroupBanPermenentModel.Api_find(group_id, user_id)) > 0 {
				GroupBanPermenentModel.Api_delete(group_id, user_id)
				go apipost.ApiPost{}.Sendgroupmsg(self_id, group_id, service.Serv_at(user_id)+"你已经脱离永久小黑屋了", auto_retract)
			}
			break
		}
		break
