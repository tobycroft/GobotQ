...
func PrivateHandle(selfId, user_id, group_id int64, message, rawMessage, remoteip string) {
	...
	if botinfo["end_time"].(int64) < time.Now().Unix() {
		iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, app_default.Default_over_time, false)
		return
	}
	if active {
		...
		if len(auto_reply) > 0 {
			...
			iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, auto_reply["value"].(string), false)
		} else {
...
func private_auto_reply(selfId, user_id, group_id int64, message string) {
	...
	for _, auto_reply := range auto_replys {
		...
		if strings.Contains(message, auto_reply["key"].(string)) {
			...
			iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, auto_reply["value"].(string), true)
			break
		}
	}
}
...
func privateHandle_acfur(selfId, user_id, group_id int64, message, origin_text string) {
	...
	case "app", "下载":
		iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, app_default.Default_app_download_url, true)
		break
	...
	case "help":
		...
		if len(botinfo) > 0 {
			if botinfo["owner"].(int64) == int64(user_id) {
				iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, app_default.Default_private_help+app_default.Default_private_help_for_RobotOwner, false)
			} else {
				iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, app_default.Default_private_help, false)
			}
		} else {
			iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, app_default.Default_private_help, false)
		}
	...
	case "绑定":
		iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, "请使用\"acfur绑定(+)本机器人密码\"来绑定您的机器人", false)
		break
	...
	case "绑定群":
		...
		iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, "您的机器人可在如下群中使用:\r\n"+strings.Join(groups, ",")+
...
func privateHandle_acfur_other(Type string, selfId int64, user_id, group_id int64, message string) {
	...
	case "密码":
		if int64(user_id) == botinfo["owner"].(int64) {
			...
			iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, "您未拥有这个机器人的权限，请先绑定机器人", true)
		}
		break
	...
	default:
		iapi.Post{}.Sendprivatemsg(selfId, user_id, group_id, "Hi我是Acfur！如果需要帮助请发送acfurhelp", false)
		break
	}
}
